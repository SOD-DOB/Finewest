...
proxyMeta = "http://%(user)s:%(pass)s@%(host)s:%(port)s" % {
    ...
    "port": '9010',
    "user": 'H6B8WY3O4988764P',
    "pass": 'B44BC8FAD71691A4',
}
...
def main():
    # 创建协程
    geventLists = []
    for id in getRules()[4000:4003]:
        g = gevent.spawn(crawl,id)
        geventLists.append (g)
    gevent.joinall(geventLists)
    # id_queue = Queue()
    # for id in getRules()[1000:1003]:
    #     id_queue.put(str(id))
    # geventLists = [gevent.spawn(crawl,id_queue),gevent.spawn(crawl,id_queue),gevent.spawn(crawl,id_queue)]
    # gevent.joinall(geventLists)
